 ...
 * @version 1.0
 */
package com.devknight.datalogger.IODevices;
...
public class mpu6050 {
    ...
    public double[] get_accel_data(boolean g) throws Exception{
        accel_x = read_i2c_word(ACCEL_XOUT0);
        accel_y = read_i2c_word(ACCEL_YOUT0);
        accel_z = read_i2c_word(ACCEL_ZOUT0);
        ...
        if(g == false) {
            accel_x = accel_x * GRAVITIY_MS2;
            accel_y = accel_y * GRAVITIY_MS2;
            accel_z = accel_z * GRAVITIY_MS2;
        }

        return new double[] {accel_x,accel_y,accel_z};
    }
    ...
    public double getPitch() {
        double pitch = Math.atan2(accel_x,accel_z);
        pitch = pitch * 2 * Math.PI;
        return pitch;
    }

    public double getRoll() {
        double roll = Math.atan2(accel_y,accel_z);
        roll = roll * 2 * Math.PI;
        return roll;
    }
